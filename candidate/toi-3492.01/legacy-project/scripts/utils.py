...
def load_config(path=None):
    ...
    path : Path or str, optional
        Path to the JSON file. Defaults to the authoritative corrected config.
    ...
    """
    if path is None:
        path = ROOT / "data" / "config_corrected_120s.json"
...
def save_config(config, path=None):
    ...
    config : dict
    path : Path or str, optional
        Defaults to ``data/config_corrected_120s.json``.
    """
    if path is None:
        path = ROOT / "data" / "config_corrected_120s.json"
