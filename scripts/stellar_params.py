...
try:
    ...
    print("  Falling back to literature values for TIC 81077799...")
    star = None
...
if star is not None:
    ...
    teff, teff_err = 5650.0, 100.0
    logg, logg_err = 4.40, 0.10
    feh, feh_err = 0.0, 0.15
    r_star, r_star_err = 0.92, 0.05
    m_star, m_star_err = 0.93, 0.05
    tmag = 12.5
...
if missing:
    print(f"  WARNING: Missing TIC values for: {missing}")
    print("  Using approximate solar-type defaults for missing values.")
    defaults = {"Teff": 5650.0, "logg": 4.40, "[Fe/H]": 0.0,
                "Rstar": 0.92, "Mstar": 0.93, "Tmag": 12.5}
    for name in missing:
        if name == "Teff":
            teff = defaults["Teff"]
        elif name == "logg":
            ...
            r_star = defaults["Rstar"]
        elif name == "Mstar":
            m_star = defaults["Mstar"]
        elif name == "Tmag":
            tmag = defaults["Tmag"]
...
try:
    ...
    print(f"  LDTk failed: {e}")
    print("  Using approximate Claret 2017 TESS values.")
    u1 = 0.30 + (teff - 5500) / 1000 * 0.08 - (logg - 4.5) * 0.05
    u2 = 0.10 + (teff - 5500) / 1000 * 0.05 - (logg - 4.5) * 0.02
    u1_err, u2_err = 0.05, 0.03
    ldc_source = "Approximate from Claret 2017 trends"
    print(f"  u1 = {u1:.4f} +/- {u1_err:.4f}")
    print(f"  u2 = {u2:.4f} +/- {u2_err:.4f}")
