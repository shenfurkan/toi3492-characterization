...
def duration_hours(rp, ar, b, ecc, omega):
    ...
    arg = np.clip(arg, -1, 1)
    t14_circ = (OFFICIAL_PERIOD / np.pi) * np.arcsin(arg) * 24.0
    ecc_factor = np.sqrt((1 - ecc**2)) / (1 + ecc * np.sin(np.radians(omega)))
    return float(t14_circ * ecc_factor)
...
def main():
    ...
    def neg_log_post(theta):
        ...
        ecc = esinw**2 + ecosw**2
        if not (0.025 < rp < 0.09 and 4.0 < ar < 13.0 and 0.0 <= b < 1.0 + rp and 0.0 <= ecc < 0.8):
            return np.inf
        omega = np.degrees(np.arctan2(esinw, ecosw)) % 360
        if ar <= b * (1 + rp) and b > 0:
            pass
        ...
        ecc_penalty = ecc * 0.5
        return 0.5 * (chi2 + ar_prior_term) + ecc_penalty
    ...
    print("Running eccentric fit with e*sin(w), e*cos(w) parameterization...")
    ...
    result = {
        "fit_type": "eccentric_free_geometry",
