...
error scale factor of 2.44 for a unit reduced $\chi^2$.  The native-cadence
model likewise finds sector radius ratios from 0.0534 to 0.0559.  Thus the
signal is persistent but its depth is not statistically constant at the
formal precision; sector-dependent depth is explicitly marginalized in the
robustness fit and remains a central systematic limitation.
...
and the preregistered K0 numerical-remediation calibration failed before any
new real-data fit.  Phase~6 therefore remains at
\texttt{FAIL\_STATIONARITY}; no residual-correlation gate, adopted geometry,
stellar-density discrepancy, or eccentricity measurement follows from this
analysis.  Native-cadence 120~s and 20~s parameter intervals are omitted, and
the converged window comparison is retained only to demonstrate unpropagated
baseline sensitivity.  A passband-integrated atmosphere/isochrone stellar
posterior and reduction-family covariance propagation are also unavailable.
